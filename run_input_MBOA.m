...
mapgrid=('slopecone_10con_3.grd');      % load a NetCDF GMT file
                                %
...
writestats=1;   % set to one to write formated stats file 
                %
figsavetype=2;  % file format to save the graphics 
                % 0 = matlab fig file 
                % 1 = PNG 
                % 2 = postscript 
...
contype=0;      % 0= use matlab's contourm 
                % 1 = use GMT's grdcontour (usualy faster). Must have GMT installed and
                % in the system path. Only tested for OSX, Linux 
                %
intpflag=1;     % set to 1 to fill small NaN data gaps in the grid  
                % 
                %
clstep=13;      % Integer contour interval (set to 1/3 to 1/2 the
                % minimum height mound you want to detect 
                % 
clstart=1;      % rounds start level of contouring to the multple of X that
...
Naz=90;            % Number of profiles to consider in a 1/2 circle 
                   % (90 gives 2 deg sectors, 180 gives 1 deg sectors)
                   %
Rstep=200;         % Range step for A/O and slope checks in meters 
...
FL=12;  % filter length for averaging or median filter (Use an odd number 3, 5 or 7) 
...
% The codes are not memory efficient (due to the fact all closed contours
% must be stored and evaluated relative to each other). For working with large grids, 
