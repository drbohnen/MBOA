...
% 
% Modified Basal Outlining Algorithm MBOA 1.0 
% DRB (NCSU) and JKH (USC) 
%
% modified on 05 Nov 2011 to fix bug in H/R ration check 
% modified 01 July 2011 to allow mult cfile directories 
%
%
% drbohnen@ncsu.edu 
...
%% BLOCK FOR PLOTTING & EVALUATION OF THE CLOSED CONTOUR FINDING
% reconstruct the C matrix. 
C_row1=[]; C_row2=[]; Slats=[]; Slons=[]; 
for q=1:length(cl);
...
Slons=cat(1,Slons,Sact(2,:)'); Slats=cat(1,Slats,Sact(3,:)');
end 
end
CNEW=cat(1,C_row1',C_row2'); plotm(double(CNEW(2,:)),double(CNEW(1,:)),'k'); plotm(double(Slats),double(Slons),'or')
...
CNEW=cat(1,C_row1',C_row2'); plotm(double(CNEW(2,:)),double(CNEW(1,:)),'w','LineWidth',3);
...
%%  BLOCK FOR PLOTTING & EVALUATION OF THE ELLIPSE CHECKS 
% reconstruct the C matrix. 
C_row1=[]; C_row2=[]; 
for q=1:length(cl);
eval(['Cact=C_' num2str(abs(cl(q))) ';']); %get active C level 
...
CNEW=cat(1,C_row1',C_row2'); plotm(double(CNEW(2,:)),double(CNEW(1,:)),'y');
...
fdz=fopen('/Volumes/G2/d_MBOA/d_MBOA1.1_Oct2011test/junk.txt','w') ; 
...
%% BLOCK FOR PLOTTING & EVALUATION OF THE H TO R CRITERIA 
% reconstruct the C matrix. 
C_row1=[]; C_row2=[]; 
for q=1:length(cl);
eval(['Cact=C_' num2str(abs(cl(q))) ';']); %get active C level 
...
CNEW=cat(1,C_row1',C_row2'); plotm(double(CNEW(2,:)),double(CNEW(1,:)),'r');
%% BLOCK 5 
...
if ~isempty(C_row1)
CNEW=cat(1,C_row1',C_row2'); plotm(double(CNEW(2,:)),double(CNEW(1,:)),'w','LineWidth',2);  % added double for linem error in R2011a
%% 
